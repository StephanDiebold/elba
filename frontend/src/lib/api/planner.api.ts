...
// ... deine Imports & Typen bleiben

const PREFIX = "/api/exam/planner";
...
const plannerApi = {
  // --- Prüfungstage ---
  listPruefungstage: async (params?: { page?: number; size?: number; von?: string; bis?: string }) => {
    const qs = new URLSearchParams();
    if (params?.page) qs.set("page", String(params.page));
    if (params?.size) qs.set("size", String(params.size));
    if (params?.von) qs.set("von", params.von);
    if (params?.bis) qs.set("bis", params.bis);
    const url = `${PREFIX}/pruefungstage${qs.toString() ? `?${qs.toString()}` : ""}`;

    // ← Backend liefert aktuell ein Array; künftig evtl. ein {items,total}-Objekt.
    const raw = await getJson<any>(url);
    const items: PruefungstagLite[] = Array.isArray(raw) ? raw : (raw?.items ?? []);
  ...
  // --- Zeitschemata ---
  listZeitschemata: async () => getJson<Zeitschema[]>(`${PREFIX}/zeitschemata`),

  // --- Slots ---
  listSlotsByPta: async (pta_id: number) =>
    getJson<Slot[]>(`${PREFIX}/pruefungstag_ausschuss/${pta_id}/slots`),

  generateSlots: async (pta_id: number, zeitschema_id: number, anzahl = 6, ueberschreiben = false) =>
    postJson<{ ok: boolean; created: number }>(
      `${PREFIX}/pruefungstag_ausschuss/${pta_id}/slots/generate`,
      { zeitschema_id, anzahl, ueberschreiben }
    ),

  changeSlotStatus: async (slot_id: number, status: "frei" | "geplant" | "gesperrt" | "abgesagt") =>
    postJson<{ ok: boolean }>(`${PREFIX}/slots/${slot_id}/status`, { status }),

  assignSlot: async (slot_id: number, pruefkandidat_id: number) =>
    postJson<{ ok: boolean; pruefung_id: number }>(`${PREFIX}/slots/${slot_id}/assign`, {
      pruefkandidat_id,
    }),
  ...
  listBezirkskammern: async (kammer_id?: number) => listBezirkskammern(kammer_id),
};
