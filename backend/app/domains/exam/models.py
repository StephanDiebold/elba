...
from sqlalchemy import Column, Integer, String, Date, DateTime, Enum, Text, ForeignKey, DECIMAL
...
class Pruefungstag(Base):
    ...
    status = Column(Enum('geplant','laufend','pausiert','abgeschlossen','archiviert'),
                    nullable=False, default='geplant')
...
class Pruefung(Base):
    ...
    status            = Column(Enum('geplant','aufgerufen','teil1','teil2','bewertung','final'),
                               nullable=False, default='geplant')
    bemerkung         = Column(Text)
